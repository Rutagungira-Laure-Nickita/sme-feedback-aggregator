...
Implementation and automated verification are complete; user-run browser/provider verification remains pending. The operational product now exposes exactly five feedback channels: Manual Entry, Public Form, QR Code, Gmail, and WhatsApp. Only Live Gmail and Live WhatsApp connections may be listed, created, authorized, activated, tested, or synchronized through normal product APIs. Historical Demo, Outlook, Google Reviews, X, Facebook, and Instagram rows and dormant adapter code remain stored for audit/compatibility, but are excluded from owner/customer/admin operational views, dashboards, reports, filters, activity, and development seed creation. Signed Facebook/Instagram webhook deliveries now receive a controlled retired-provider response without ingestion; the signed WhatsApp path remains active.
...
Implementation and automated verification are complete; the new migration and user-run browser/security verification remain pending. Business Owners and Business Admins can edit allowed feedback/customer snapshot fields, change status/category, soft-delete one or many records, select the current page or every matching filtered record, and use an exact `DELETE` confirmation before removing all feedback. Provider channel, external IDs, ingestion/deduplication records, source metadata, and original timestamps remain immutable. Soft-deleted feedback and its related ingestion, workflow, AI, automation, and audit records are preserved but excluded from ordinary inboxes, customer views, dashboards, AI/automation work, and owner/platform reports.
