...
Business Owner reporting intentionally has two consumers of the same authorized backend document. The Overview dashboard continues to consume the rich preview metrics. The Reports page filters that preview to `Detailed Feedback Records`, and the export controller applies `simplifyBusinessOwnerReportForExport` before the shared PDF/CSV renderer. The page and PDF also show a four-channel `Feedback by Channel` summary counted directly from all detailed rows before preview truncation; the export projection enables it only for PDF. CSV retains concise scope/filter metadata and the six detail columns. No additional query or shared-renderer change is needed.
